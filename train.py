...
def main():
    ...
    def compute_metrics(eval_preds):
        ...
        results = (preds == labels).sum() / preds.shape[0]
    ...
    if model_args.encoder_name_or_path is not None:
        ...
        encoder_config.num_cross_attn_layers = config.num_cross_attn_layers
    ...
    if training_args.do_train and "cepe" in model_args.model_class:
    ...
    def train_param(param_name):
        ...
        if model_args.train_encoder and "encoder" in param_name:
            return True
        if model_args.train_embedding and "embed" in param_name:
            return True 
        if "emb_proj" in param_name: # defrozen emb_proj layer
            return True
        # FIXME: Reset
        return "cross_attn" in param_name
